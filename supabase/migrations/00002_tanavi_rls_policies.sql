...
CREATE POLICY "Admin write products" ON public.products FOR ALL USING (is_admin_or_staff(auth.uid()));
CREATE POLICY "Admin write categories" ON public.categories FOR ALL USING (is_admin_or_staff(auth.uid()));
CREATE POLICY "Admin write collections" ON public.collections FOR ALL USING (is_admin_or_staff(auth.uid()));
CREATE POLICY "Admin write product_collections" ON public.product_collections FOR ALL USING (is_admin_or_staff(auth.uid()));
CREATE POLICY "Admin write product_variants" ON public.product_variants FOR ALL USING (is_admin_or_staff(auth.uid()));
CREATE POLICY "Admin write product_images" ON public.product_images FOR ALL USING (is_admin_or_staff(auth.uid()));
CREATE POLICY "Admin manage inventory" ON public.inventory FOR ALL USING (is_admin_or_staff(auth.uid()));
CREATE POLICY "Admin view/manage inventory_movements" ON public.inventory_movements FOR ALL USING (is_admin_or_staff(auth.uid()));
CREATE POLICY "Admin manage orders" ON public.orders FOR ALL USING (is_admin_or_staff(auth.uid()));
CREATE POLICY "Admin manage order_items" ON public.order_items FOR ALL USING (is_admin_or_staff(auth.uid()));
CREATE POLICY "Admin manage payments" ON public.payments FOR ALL USING (is_admin_or_staff(auth.uid()));
CREATE POLICY "Admin manage payment_events" ON public.payment_events FOR ALL USING (is_admin_or_staff(auth.uid()));
CREATE POLICY "Admin manage order_status_history" ON public.order_status_history FOR ALL USING (is_admin_or_staff(auth.uid()));
CREATE POLICY "Admin manage coupons" ON public.coupons FOR ALL USING (is_admin_or_staff(auth.uid()));
CREATE POLICY "Admin manage announcements" ON public.announcements FOR ALL USING (is_admin_or_staff(auth.uid()));
CREATE POLICY "Admin manage shipping_methods" ON public.shipping_methods FOR ALL USING (is_admin_or_staff(auth.uid()));
CREATE POLICY "Admin manage site_settings" ON public.site_settings FOR ALL USING (is_admin_or_staff(auth.uid()));
CREATE POLICY "Admin manage audit_logs" ON public.audit_logs FOR ALL USING (is_admin_or_staff(auth.uid()));
